...
### 票 299 — 「小说」正名「书籍」+ 分类池按组隔离（体裁）

- [x] TYPE_GROUPS/GROUP_ORDER/TYPE_COLORS 组名 小说→书籍（色值沿用）；LEGACY_TAG_MAP 加 小说→书籍（读侧归一，fm 原值不改写，同票 293 先例）
- [x] 题材池按组隔离：data.json 新键 entertainmentGenresByGroup（settings 接口+默认 {}）；书籍默认体裁=中图法 22 大类（去顿号规范名），其余六组保持 悬疑/爱情/年代；旧全局键缺省回落（剔除旧默认三项防污染），组键写入即固化
- [x] 体裁文案跟随（纯显示层，fm 键「类型」不动）：表单行标签（切类型即时重绘 j-genre-label）、详情弹窗、导出感想卡；添加提示语跟随
- [x] recommend.ts 提示词与 GROUP_DEFAULT_TAG 跟随（书籍:'书籍'，小说:'书籍' 兼容归一）；home/settings-panel 副标题同步
- [x] options.test 重写（按组隔离/回落/固化/幂等 8 例）；ui.test 补体裁切换用例；data.test/douban-queue.test 归一口径更新
