...
**待办日记投影 (Todo Diary Projection)**: 当天日期文件 `## 代办事项` 小节里的待办行集合，是 memo.json 未完成条目的当日投影。行格式 `- [ ] <序号>. <标题>-<HH:mm>`（完成 `- [x] …`）。数据本体永远在 memo.json，投影行不承载独立数据。
_Avoid_: 投影行当数据源；日记里的待办=另一份待办数据（它只是投影）
...
**顺延 (Rollover)**: 插件启动/当天首次打开待办面板时，把所有历史未完成且今天无投影行的条目补写进今天 `## 代办事项` 小节（去重只补一次，按原创建时间排序接续序号）。无后台进程，当天不启动则不顺延、下次启动补齐。
