...
**类别 (Category)**: 待办条目的「必须 / 想要」二值标记（`must`/`want`），手动标，缺省「必须」。三维分类体系的一维。
_Avoid_: 场景（场景是剪藏/工作等归档维度，与此无关）；标签

**待办紧急度 (Urgency)**: 待办条目的「紧急 / 不紧急」二值标记（`urgent`/`not`），手动标，缺省「不紧急」。UI 标签沿用用户口径叫「时间」。
_Avoid_: 与截止时间 (due) 混淆——due 是具体日期时刻，紧急度是主观标记，两者正交（远期 due 可标紧急）；从 due 自动推导

**分类器 (Classifier)**: 待办面板工具行的下拉，11 项 = 全部 + 类别×优先级×紧急度 8 组合（如「必须/重要/紧急」）+ 按到期 + 按创建。选组合 = 三维等值筛选；末两项 = 全部 + 对应排序。原「紧急优先」排序下拉被它替换，其排序逻辑（到期优先 + 重要优先）保留为内部默认排序。筛选选择不跨开合记忆，每次打开重置「全部」。
_Avoid_: 排序器（组合项是筛选不是排序）；把它叫「紧急优先」

### 待办日记同步（ADR-0122）

**待办日记投影 (Todo Diary Projection)**: 当天日期文件 `## 代办事项` 小节里的待办行集合，是 memo.json 未完成条目的当日投影。行格式 `- [ ] <序号>. <标题>-<HH:mm>`（完成 `- [x] …`）。数据本体永远在 memo.json，投影行不承载独立数据。当天文件缺失时按日记模板 `CONFIG/TEMPLATE/模板-日记.md` 建骨架再落投影（`seedWhenMissing`，与日程规划同源，ADR-0123）。
_Avoid_: 投影行当数据源；日记里的待办=另一份待办数据（它只是投影）；建档时另拼一份骨架（骨架由模板单点提供，ADR-0123）

**日记行序号 (Diary Line Number)**: 投影行在**当天写入**的先后顺序号，按天重排不跨天稳定；新行接当天既有最大号续号，已写下的行永不重排。由写入顺序推导，不落 memo.json。
_Avoid_: 条目 ID（序号不是身份，跨天会变）；面板序号（面板不显示）

**同步窗口 (Sync Window)**: 待办↔日记双向同步只作用于**今天**的日期文件；历史日记是当天状态快照，永不回改、不触发反向。
_Avoid_: 全历史双向同步

**顺延 (Rollover)**: 插件启动/当天首次打开待办面板时，把所有历史未完成且今天无投影行的条目补写进今天 `## 代办事项` 小节（去重只补一次，按原创建时间排序接续序号）。无后台进程，当天不启动则不顺延、下次启动补齐。今天文件不存在时按日记模板 `CONFIG/TEMPLATE/模板-日记.md` 建骨架（**不再**用「标记+首行」的裸建；ADR-0123）。小节已存在但无新行时**原样返回、绝不重建小节**（旧缺陷会天天在文末长出空行，ADR-0123 修）。
_Avoid_: 自动移动待办到明天（是「今天补投影」，不是改 due/搬数据）
...
**复习计划 (Review Plan)**: FSRS v4 算法驱动的复习管理，数据 `CONFIG/STORAGE/review.json`——**ticket 168 起收敛为纯数据层（ADR-0077）**：数据文件 + 监听收编 + 到期提醒，交互入口全部收敛到「复习（按数量）」篇数弹窗 + ⚙️ 设置（「开始复习/复习计划」等命令与复习主面板整体退役，注册命令 39 → 35；ticket 169 加回手动入口「将当前文档加入复习计划」`bz-review-add-current`——命令面板/快捷键/文档内右键待选三表面，仅 .md 可加，重复加入不动既有排期，注册命令 35 → 36；ticket 170 加批量入口「批量加入复习计划」`bz-review-add-current-with-links`——当前文档及其一级出链（正文 + frontmatter 链接，断链/非 .md 丢弃）逐篇加入，已在计划中的跳过不动排期，结束给汇总通知，注册命令 36 → 37）。可配置多个「监听文件夹」自动收编笔记；做题会话自动评级未通过（忘了/困难）时结果卡变唯一按钮「复习此笔记」并置「待重做」，按数量复习会话内重做篇通过才清标记（首次评级=唯一排期来源，ADR-0044）；「做题家」命令入口已退役（ADR-0045），仅作复习引擎。ticket 100 起：**到期提醒**（enableAutoNotify，开启时插件启动即常驻轮询，有逾期笔记即弹聚合通知；通知「去复习」action——ticket 168 起直接进全 vault 逾期按数量会话，不再单篇跳转）；**每日复习上限**（reviewDailyLimit，一轮复习最多处理 N 篇逾期）；**复习间隔缩放**（reviewIntervalScale，FSRS 相位间隔乘系数，ADR-0046）；**文件树标记**（reviewTreeBadge，关闭则文件树不染色不挂徽章）；**自动加入提醒**（reviewAutoAddNotice，新笔记自动收编时 3 秒窗口合并一条通知）。⚙️ 设置弹窗（唯一设置入口）含「复习条目管理」组：挂起条目标灰、逐条移出确认 → 撤销原样恢复（阶段/排期/历史零丢失，清理 review.json 的唯一入口，ADR-0077）。

**做题家 (Quiz Master)**: 统一题库 `CONFIG/STORAGE/quiz.json`，多选支持，完成状态记录，自动替换全完成的笔记。命令入口 `bz-quiz-open`/`bz-quiz-update` 已删除注册（ADR-0045），仅经复习计划按数量复习会话驱动（startReviewSession/endReviewSession 契约）；多选答对计数已修复（ADR-0044 唯一解冻项）。ticket 153：作答节奏拍板——**答对自动进入下一题**（持久化成功后直达，不出现「下一题」按钮），**答错才显示「下一题」按钮**（点按或 Enter）。ticket 168：复习全面做题化，「用做题测难度」开关退役（forceQuizForReview 键删），4 出题子项常显。ticket 176：**出题链路单篇化**（ADR-0079）——「更新题库」模块直调入口（quizUpdate/updateQuiz）与批量键回显协议（generateBatch）整体退役，出题一律逐篇生成+即时落盘；AI 请求带 90s 超时与瞬时失败退避重试（quiz 层包装，不动 core），显式关思考（enable_thinking:false），单篇校验改逐题过滤；**选项打乱**——生成落盘前选项乱序并重映射 correctIndices，消除模型正确答案位置偏好；提示词修多选矛盾、截断统一 10000 字符、追加选项卫生与内容依据约束；questionsPerNote 钳制 1~20；**出题数量按篇幅自适应**——设置留空时按截断后篇幅四档提示（<500 字 2~3 / <2000 3~5 / <5000 5~8 / 其余 8~12，`countRangeForLength`），显式数字仍恰好 N 道优先；core/ai.ts 增 response_format 400 去字段降级重试（五域共享）。
...
**今日已复习 (Reviewed-Today List)**: 用户可随时调出的当天复习清单（命令「今日已复习」）——把「当天已复习」的条目按最后复习时间倒序罗列（附当天复习次数与最后评级），点击可在阅读模式下查看原文，用于复习后的回看与抽查。清单是只读视图，不改动任何复习数据。
_Avoid_: 复习历史（指单条笔记的完整历史时间线，不是当天清单）

**复习阶段采样 (Stage Sampling)**: 按数量复习在安排「已复习笔记」时的轮询采样——按 `stageBucket`（0–2 刚学 / 3–6 短期 / 7–9 中期 / 10+ 长期）分组，桶间轮转取篇，避免一次复习集中在同一阶段。
...
**正文指纹 (Body Fingerprint)**: 第二大脑增量索引的内容判定依据（ticket 173/ADR-0078）——对已存 chunks 文本拼接的 FNV-1a 哈希，口径与向量输入严格一致（frontmatter 已剥、标题已并入首块）。三层判定：mtime 未变直接跳过；mtime 变但指纹同（改 YAML/纯触碰）只更新登记不重嵌；指纹变（改正文/改名）才重嵌。挪动文件夹 = 旧路径条目成孤儿，新路径指纹命中孤儿池即继承向量、迁移登记键，全程不调嵌入服务；孤儿池只在单轮刷新内存活，未命中照常清理。_Avoid_: 内容哈希（指 link agent 正文基准哈希时不用此词，那是建链重跑判定）、全量重建
...
**顶级类型 (Type)**: 娱乐条目的固定七项分类：电影/电视剧/短剧/书籍/动漫/纪录片/公开课（ADR-0127 七项体系；**ADR-0128 起原「小说」正名「书籍」**，旧 tag 经 LEGACY_TAG_MAP 读侧归一显示，fm 原值不改写；「剧集」组退役、细分 tag 归一映射为「电视剧」）。**不开放自定义**（牵动集数与豆瓣抓取行为）。落盘仍是 frontmatter tags。
_Avoid_: 「小说」作为类型名（正名「书籍」后仅作旧数据 tag 存在）
...
**题材 (Genre)**: 娱乐条目的多选分类，复用 fm 键「类型」（顿号分隔字符串）。默认待选项按组改定（票 299 同日补充）：影视四组（电影/电视剧/短剧/动漫）=剧情/喜剧/动作/爱情/科幻·奇幻/悬疑·惊悚/历史·传记/纪实/动画/其他；纪录片=历史/人物传记/自然地理等 16 项；公开课=学科门类 12 项；书籍（显示为「体裁」）=中图法 22 大类。自定义项**按组隔离**（data.json `entertainmentGenresByGroup`，组 → 自定义段；票 299 起旧全局键 `entertainmentGenres` 仅作组键缺省回落，存量自定义项全组可见）。豆瓣抓取的题材串按分隔符兼容读取。
_Avoid_: 与顶级类型混用（题材是内容属性，类型是形态分类）；跨组共享自定义项的旧假设（票 299 起按组隔离）

**体裁 (Book Genre)**: 书籍组条目的题材行专名（图书馆口径，票 299 / ADR-0128）——行标签、详情弹窗、导出卡在书籍条目下均显示「体裁」，其余组仍「题材」。默认待选项为《中国图书馆分类法》22 大类的去顿号规范名（类名内不得出现存储分隔符「、」）。数据同「题材」：fm 键「类型」，书籍组隔离池。
_Avoid_: 把体裁当独立数据键（与题材同键同池机制，仅显示层区分）
...
**关闭方式 (Off Mode)**: 小橘电源开关的关闭语义三档（ticket 103，插件设置页「启用小橘」开关 +「关闭方式」下拉，立即生效）——**彻底停机 (stop)**：全量卸载零感知（unloadSmartCat：容器/监听/定时器/记忆流累积全停，重启不挂载；关闭期间笔记活动不进记忆流且不补记）；**仅隐藏 (hide)**：猫 DOM 收起后台感知照跑（重启「隐藏启动」装配——`startHidden` 旗标：子系统在线、容器不出现、问好不闪现）；**仅不自动启动 (lazy)**：仅约束启动时机（当场零变化，本次会话不打扰；重启不自动挂载，命令召唤即启动）；默认 `stop`；重新开启=装配显示或幂等重挂（openSmartCat 重挂路径），开关循环 smartcat.json 零改动零迁移；停机关开关顺带清理小橘 ⚙️ 设置弹窗（closeSettingsModal）。**隐藏 ≠ 关闭**——「隐藏小橘」命令（及 hide 档）只是收起容器，感知不中断；停机才是真关。
_Avoid_: 关闭/隐藏混用；停机档命令自动复活（召唤/聊天被拒并提示「小橘已在设置中关闭」）；⚙️ 弹窗加开关（仅插件设置页）；只关 AI 大脑模式（Q4 否决另票）
...
**分步写日记 (Two-Step Add Diary)**: 写日记弹窗的两步形态（ADR-0109）——**第一步「类型 + 时间」**（`#add-diary-popup`，吸底「下一步」，未选类型拦截于冻结文案「请至少选择一个类型」）、**第二步「正文」**（`#add-diary-content-popup`，吸底「上一步」+「保存」）。两步共用一个 `#add-diary-mask`，第一步隐藏而非销毁，故 datetime 与类型容器连同选中态始终是 `saveNewEntry` 的数据源。移动端起因：原单弹窗正文框位于类型区下方，软键盘弹起后正文与保存被吞；两步后第二步只有「一个 textarea + 两枚按钮」，不做键盘避让亦无遮挡。草稿语义：「上一步」保留，「遮罩取消 / 保存」丢弃。preset 已带分类的入口（如每日触动点记录预选「触动点」，票 302 前为每日复盘/复盘）跳过第一步。
_Avoid_: 「写日记弹窗」单指第一步——它是两步的合称，指具体某一步要写明第几步

**日记内容块 (Diary Content Block)**: 写日记与每日触动点记录（原「每日复盘」，票 302 更名）的落盘单位（issue 252，ADR-0114）——不再是 `# emoji HH:mm` 条目，而是往骨架小节里追加的**多行块**。写日记 → `# 随笔` 小节，块首行 `**✍️ 20:30**`（**加粗行不是标题**；多类型 emoji 连写），故仓库里四份头条正则（`diary/parser.ts`、`diary-wall/parser.ts`、`smartcat/diary-source.ts` 及 `recap` 复用的 `parseFile`）一律不命中，内容不进日记面板列表 / 标签筛选 / 回忆墙 / smartcat。每日触动点记录 → `# 当日触动点` 小节（票 302 更名，原 `# 当日复盘`；**旧落点不兼容**——旧文件的 `# 当日复盘` 不再命中追加，历史小节一字不动），块首行 `## 🪞 HH:mm`（h2），其下接触动点记录内层模板（`### 触动点` 起，`REVIEW_TEMPLATE` 已去首行以避免小节标题重复）。落点小节缺失时的行为**有意不对称**：`# 随笔` 在骨架里 → 缺失不建、追加文末并弹 warning（存量文件一字不改）；`# 每日触动点记录`（票 302 二次裁定前曾用此标题，后定 `# 当日触动点`）不在骨架里 → 缺失则写时在文末新建。插入实现与捕获行共用 `daily-capture.ts` 的 `locateSection`（`insertBlockIntoSection` vs `insertIntoSection`），「新层级优先、旧层级兜底」与「小节边界取命中行实际层级」只有一份。写盘走 core/storage 同路径串行队列（键 = 日记文件路径），与 `writeFile` 互斥。副作用：这些内容**不能**在插件面板里编辑/删除，只能直接开文件改；`parseFile` 与条目模型保留（回忆墙 / smartcat / 今日回顾 / 加密域仍要读存量条目）。_Avoid_: 把写日记的块首行写成 `# emoji HH:mm`——会被那四份头条正则当条目解析进面板/回忆墙/smartcat，与用户「不想在那些地方看到」的诉求直接冲突；判据是加粗行（非标题），不要换成任何层级的标题。_Avoid_: 给 `# 随笔` 也做「缺失即建」——会在存量条目形态文件里凭空长出小节，违反「存量不动」；只有 `# 当日触动点`（票 302 更名，原 `# 当日复盘`；旧落点不别名兼容）允许现场建节。_Avoid_: 把「检测日记解析」工具加回来——用户明确要求取消（ADR-0114）；写前守卫保留但不再引导到任何工具
...
**日常时间记录 (Daily Time Log)**: diary 域 `src/diary/daily.ts`（ticket 245，自 vault `CONFIG/SCRIPTS/日常时间记录/` 三个 QuickAdd 宏整合进插件）与 `src/diary/daily-capture.ts`（issue 247，同源 Multi 剩下两个 Capture 宏换血）——五件事：**当天任务完成情况**（7 项每日任务经 `openFlowDialog` 依次打勾，状态沿用 `CONFIG/SCRIPTS/每日任务状态.json`，`{ "YYYY-MM-DD": { taskId: boolean } }` 格式零迁移，路径硬编码不走 storagePath；取消=跳过不计，写回走 `updateFileSections` 段级合并只声明今天一段）、**每日触动点记录**（原「每日复盘」，票 302/ADR-0131 更名；`REVIEW_TEMPLATE` 经写日记弹窗 preset 预填，**落 `# 当日触动点` 小节**（票 302 更名，原 `# 当日复盘`；旧落点不兼容，缺失则写时在文末新建，已有则按时间顺序追加节内末尾）；块首行 `## 🪞 HH:mm`，多条按时间顺序堆叠；ADR-0114 起不再作为日记条目写入；标签「触动点」（票 302 前为「复盘」，emoji 🪞 不变）、**日程规划**（入口 `openPlanPicker`：`openFlowDialog` 两动作选「当日 / 明日」，默认焦点在明日 → `planDiary(target)` 按日记模板 `CONFIG/TEMPLATE/模板-日记.md` 为对应日期**建文件**：**日程三段以模板为唯一真源（ADR-0123）**——`## 代办事项`/`## 完成情况跟踪`/`## 备注` 直接写在模板 `# 日程规划` 之下，`extractPlanBlock(模板)` 原样取用（模板缺 `# 日程规划` 才回退内置 `buildPlanContent()`；三段已去掉原先三个 `- [ ] ` 空占位）。文件不存在 = 模板原文落盘（frontmatter 规整，不再另拼三段）；已存在但无规划 = 文件末尾补写 `extractPlanBlock(模板)`；已存在且有规划 = 提示不重复、一个字节不写。两目标**共用同一模板**，只有落盘日期不同；判重 = **文件原文**同时命中 `PLAN_MARKER`(`## 代办事项`) 与 `PLAN_TRACK_MARKER`(`## 完成情况跟踪`)，ADR-0111/0112；**标题层级自 ADR-0113 起整体上提一级**——日记骨架 `#`（`# 睡眠相关`/`# 随笔`/`# 新闻联播内容记录`/`# 日常行为记录`/`# 日程规划`）、日程三段 `##`、触动点记录首行 `# 当日触动点`（票 302 更名，原 `# 当日复盘`；其下 `###`/`####` 不动）；读侧（判重 / 补写 / 捕获插行）按标题**文字**匹配、层级无关，旧日记的 `##`/`###` 照常命中）、**日常行为记录**（issue 247，输入活动 → `- HH:mm-活动` 插进 `# 日常行为记录` 小节末尾）。捕获为**捕获行**语义：不经 `addEntry` 条目链路，文件级原样插行其余行不动（与 QuickAdd 宏**同格式共写**，同款先例见 collect 域；小节未命中=文末整行追加，文件缺失=「标记+首行」新建），写盘走同路径串行队列与 `writeFile` 互斥。入口：命令 `bz-diary-task-check`/`bz-diary-review`/`bz-diary-plan`/`bz-diary-activity-capture` + 日记面板头部 📋/🪞/🗓️/🏃（「当日待办事项」捕获 `bz-diary-todo-capture` 与 ✅ 按钮已随 ADR-0122 退役，`## 代办事项` 唯一写入者 = todo 域日记同步）。
...
_Avoid_: 把日程三段硬编码在 `buildPlanContent()` 当主源、或让「日程规划」与「待办顺延」两条建档路径各拼一份——模板 `CONFIG/TEMPLATE/模板-日记.md` 才是唯一真源，两路都走 `extractPlanBlock(模板)`；一旦各自为政就会重现「同名两个 `## 代办事项` / 骨架缺失 / 正文落到文末」（ADR-0123）
