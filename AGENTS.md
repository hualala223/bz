...
- 用户只打命令片段（如 `/g`、`/story-out`）时，不要质疑其不完整：先从可用 skills 列表按前缀/子串匹配，给出候选表（序号+命令+一句话用途），等用户选定后再用 Skill 工具调用。
...
- 产物直出真实 vault 的插件目录 `N:/仓库/仓库-新/-0.笔记汇总库/.obsidian/plugins/bz/`（`scripts/vault-dir.mjs` 单点定义，esbuild.config.mjs 与 build-css.mjs 共用）；`pnpm run build` 另把发布版 `main.js`（minify）同步到仓库根目录，`styles.css` 聚合时即写根目录——根目录三件套是发 GitHub Release 的版本
