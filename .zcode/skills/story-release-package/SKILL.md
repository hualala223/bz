...
> 本 skill 的前置条件（`qa.review.publish` 回执、`artifact_version_id`、`candidate_hash`、Run Envelope）按托管产品形态编写。在 zcode 本地明文项目中，编排器/ArtifactFS 不存在，按以下方式降级：
> - `qa.review.publish` 回执 → 用本地已通过的发布级审查记录替代：先跑 `/story-review` 或 `/story-final-check`（发布前单章走 review、卷末/完本走 final-check），确认无未解决 S1/S2 与 blocking 命中，并把审查结论（`追踪/体检_*.md` 的八项总检为「可进入投稿」或 review 报告 P0/P1 清零）作为发布前回执。
> - `artifact_version_id` → 本地用「正文文件相对路径 + sha256」代替；`candidate_hash` → 生成发布清单时对输入正文文件逐项算 sha256 记录。
> - 任一正文文件在本轮验收后变更 → 旧回执/哈希失效，须重跑 review/final-check 并重算。
> - 若在具备 Run Envelope/ArtifactFS 的托管环境运行，则按下方「前置条件」「输入契约」执行。
