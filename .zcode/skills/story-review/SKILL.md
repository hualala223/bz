...
6. **确定性预检（只报告，不修改）**：当审查范围包含本地正文文件路径时，运行本 skill 自带脚本：
   ...
   - `check-ai-patterns.js` 的 findings 合并进 `prose`：severity=blocking 的类别一律按 S2（当前为 `not-is-comparison` / `voice-contrast` / `negation-parade` / `reverse-not-is` / `trailer-ending` / `trailer-summary`），修法直接采用检测器输出的建议（删否定铺垫/反差腔/排比否定/章尾预告腔/章尾状态总结句，直接写后项或具体动作）。`em-dash` 在脚本中为 `advisory`，只按语境判断（打断→动作 beat/短句、拖长音→省略或动作、插入说明→逗号/冒号），不一律升 S2，也不一律改句号。
...
- 提示指令：
  ...
  涉及身世/血缘/亲属/婚姻/传承/所有权/权限/不可逆规则时，必须以 `追踪/长期事实.md`、`追踪/关系清单.md`、`追踪/事实档案/{实体}.md` 为索引，沿 evidence 回读原始正文/设定；检查单值事实、禁止误读、作者真相和读者揭示范围。
  项目路径：{项目根}
  审查范围：{文件路径/章节/必要摘录}
  已知角色：{从设定文件提取角色列表}
...
> **边界说明**：本 skill 只维护**追踪事实**（经 `tracking_commit.py mode=revision`）；涉及已提交正文、大纲或设定的**实质内容**修改，不得在本 skill 内直接改，须转对应写作 skill 的「跨产物修订门禁」（AGENTS.md：`revision-governor phase=plan` → `追踪/修改影响/active.json` → `revision_guard.py check`）走完审批后才动。审查只产出修改建议，不越权改正文/大纲/设定。
