...
4. 读取核心发现：结构段落、情感曲线、反转位置、铺垫方式、句式节奏、可借鉴技法。**把拆文报告里的具体招式对到题材包招式库**：拆文给「这一篇怎么做的」，题材包给「这一类通用怎么做」，两者合用——拆文是当前对标书的实证，题材包是该题材的通法
...
| 定稿后 | 用户要求「准备发布/写书名简介标签/生成发布材料」时，路由到 `/story-release-package`（不在此生成发布文案；需先通过发布级审查） |
