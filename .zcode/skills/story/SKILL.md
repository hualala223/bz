...
| 开书设定 | 开书、开书设定、从灵感开始、搭世界观、定题材、定核心情绪 | `/story-openbook`（按《小说设置提示词》G1-G2 逐决策点确认，产物落 `设定/` 后交棒 `/story-long-write`） |
| 写长篇 | 写大纲、长篇、连载、日更、续写 | `/story-long-write` |
| 写短篇 | 短篇、盐言、一万字 | `/story-short-write` |
| 人物设计 | 人物深度设计、设计主角、设计反派、做深度人设、人物交叉核查 | `/story-character-lab` |
| 约束清单 | 出约束清单、列设定约束、扫描设定 | `/story-constraint-gate`（前序成果 → A 硬/B 软/M 三表 → 确认 → `大纲/_约束清单.md`） |
| 场景卡 | 场景卡、分场景、按场景卡写第N章 | `/story-scene-card` |
| 定文风 | 定文风、语言风格设计、人物语言特征卡、重设风格 | `/story-style-spec`（产出 `设定/文风.md`） |
| 完稿体检 | 完稿体检、卷末体检、全书体检、完本检查 | `/story-final-check` |
| 数据分析 | 数据分析、后台数据、推荐数据、在读人数、跟读率、读完率 | `/story-data-analyze` |
...
| 逐步确认 | 逐步确认、逐章确认、快速模式 | 写作类任务默认叠加 `/story-stepwise`（全程确认门）；用户明示「快速模式」解除，仅保留各 skill 自带停靠点 |
...
3. 分析其他用户请求，提取意图关键词。
4. 匹配上表，找到对应的 skill。开书类请求（开书设定/从灵感开始/搭世界观/定题材）优先 `/story-openbook`；写长篇/日更/续写/写大纲才落 `/story-long-write`。写作类任务自动叠加 `/story-stepwise`（确认门），无需用户额外点名。
...
- `story-explorer` 不可用 → 主线程直接用 Read/Grep 从项目文件检索（角色状态/伏笔/进度/设定），回答前标注 `Fallback: agent unavailable -> direct lookup`；项目尚未部署时提示先 `/story-setup`（Codex 中用 `$story-setup`）。也可路由到 `/story-explore` skill（同名只读 Skill，见路由表）按既定 query_type 检索。
- `story-researcher` 不可用 → 主线程用现有检索/回答能力完成，或提示用户改用 `/browser-cdp` 采集，同样标注 `Fallback: agent unavailable -> direct lookup`；也可路由到 `/story-research` skill（同名带来源查证 Skill）。
