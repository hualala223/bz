...
| `拆文库/{对标书名}/原文/*.md` | `{项目}/对标/{对标书名}/原文/*.md` | 原文分章视图：story-long-write 读 `对标/{书}/原文/第{N}章_*.md` 做逐章对照（analyze 管道前置会把原文切/存到 `拆文库/{对标书名}/原文/`；若为单文件 `原文/原文.txt`，复制时按章节拆分或按需映射） |
...
短篇对标产物映射（与长篇 Step 9 同口径，但短篇无 `剧情/节奏.md`/`情绪模块.md`，不套用长篇缺失处理）：

| 源路径 | 短篇对标路径 | 用途 |
|---|---|---|
| `拆文库/{对标书名}/拆文报告.md` | `{标题}/对标/{对标书名}/拆文报告.md` | 人类可读摘要投影：故事核/结构/情感线/反转/共鸣 |
| `拆文库/{对标书名}/情节节点.md` | `{标题}/对标/{对标书名}/情节节点.md` | 结构/节拍参考 |
| `拆文库/{对标书名}/写作手法.md` | `{标题}/对标/{对标书名}/写作手法.md` | 技法参考（POV/对话/时间/信息） |
| `拆文库/{对标书名}/_meta.json`（可选） | `{标题}/对标/{对标书名}/_meta.json` | 结构计数/口径参考 |

缺失处理：`拆文报告.md`/`情节节点.md`/`写作手法.md` 任一缺失 → 报告并提示对 `{对标书名}` 重跑 `/story-short-analyze`；不生成半套短篇对标视图。短篇对标仅供 story-short-write 读取，不按长篇 Step 9 的 `剧情/节奏.md` 必选门槛校验（短篇拆文不产这两个文件）。
