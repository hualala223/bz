...
1. **情绪类型分布**：当前哪种情绪拉扯最火——按 story-short-write 的六类目标情绪口径归类（意难平/反转震撼/爽感释放/治愈温暖/细思极恐/共鸣感动），并把市场常用说法（虐恋/反转/悬疑/治愈/打脸等）映射到六类下，便于下游 write 直接按情绪消费
...
**落盘**：把 `# 短篇网文扫榜报告` 写入**本次扫榜输出目录** `{outdir}/`（文件名 `短篇扫榜报告_{平台}_{YYYYMMDD}.md`，平台为多平台时用 `短篇扫榜报告_{YYYYMMDD}.md`）。原始采集数据（`{平台}{类型}_{YYYYMMDD}.md`，见前）与报告放同一 `{outdir}/`。若用户在项目目录内运行且已有 `追踪/` 或 `设定/`，则写到项目根下 `扫榜/` 子目录，避免污染正文工程。
...
**选题决策落盘**：把最终推荐选题（能爆的原因 → 市场验证 → 差异化 → 可行性+风险+验证动作，2-3 个）写入本次扫榜输出目录 `{outdir}/选题决策.md`（与 long-scan 同一契约，方便 story-short-write/story-long-write 复用）。告知用户路径与下一步：开书时若小说项目与 `选题决策.md` 同级或共用上级目录，`/story-short-write` 或 `/story-long-write` 会自动找到并确认；隔得远就把它复制到项目根，或开书时把路径粘给它。想确认"能爆的原因"先 `/story-short-analyze` 拆对标作品。
