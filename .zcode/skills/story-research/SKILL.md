...
**晋升落盘（供 long-write Phase 4 消费）**：研究报告通过门禁后，把 Findings 整理为人类可读 markdown，落盘 `参考资料/{topic}.md`（`topic` 用无空格的小写/中文规范化主题名），并登记到 `追踪/` 或项目索引（如存在）；long-write 在 `参考资料/{topic}.md` 消费。若当前环境未提供可落盘的 `参考资料/` 目录（纯 Candidate 运行），在输出中标注 `reference_landing: unavailable` 供编排层处理，不静默丢弃。
