...
4. 整书源文件为单文件（`原文/原文.txt` / `原文.md`）且按章切分可行时，按章节标题拆分出 `原文/第{N}章_*.md`（`第001章_章名.md` 编号规则与下游一致），供 story-long-write 读 `对标/{书}/原文/第{N}章_*.md` 逐章对照；无法可靠切分时保留单文件，并在 `_meta.json` 记 `original_split: "single"` 供 import/写作侧按需映射
5. 备份完成后验证：
...
│   └── 第001章_章名.md   # 可选：按章拆分的分章原文（无法切分时省略）
├── 概要.md
├── 章节/
│   ├── 第1章_深度拆解.md
│   ├── 第2章_深度拆解.md
...
- 主线程调用确定性 renderer：
  ...
  "$PYBIN" .zcode/skills/story-long-analyze/scripts/render_chapter_summary.py \
