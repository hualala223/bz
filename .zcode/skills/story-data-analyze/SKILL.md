...
> **首次运行/文件缺失引导**：若 `数据追踪/` 目录不存在或 `拉取番茄数据.py` 缺失，报错并提示「首次运行需初始化数据追踪目录」——检查项目是否已部署（`story-setup`）并确认存在 `数据追踪/拉取番茄数据.py`、`数据追踪/knowledge/index.json`、`数据追踪/番茄数据日志.md` 三个运行时产物；缺失时让用户提供数据源或确认是否由外部脚本/上传填充，不得凭空生成占位数据冒充官方口径。
