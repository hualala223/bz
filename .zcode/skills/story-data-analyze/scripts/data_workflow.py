...
try:
    import fcntl  # POSIX-only; Windows falls back to msvcrt below
...
def _safe_unlink(path: pathlib.Path, attempts: int = 5) -> None:
    """Best-effort temp cleanup: retry transient Windows locks, ignore absence."""
    for i in range(attempts):
        try:
            ...
            try:
                os.chmod(path, 0o666)  # Windows: hard-linked 0o444 temp needs write bit cleared
            except OSError:
                pass
            if i == attempts - 1:
                return  # temp residue is non-fatal; artifact was already committed
            time.sleep(0.05)
...
def run_lock(runs_root: pathlib.Path, run_id: str) -> Iterator[None]:
    ...
    with lock_path.open("a+b") as handle:
        ...
        else:  # Windows: msvcrt byte-range lock (ensure file is non-empty first)
            import msvcrt
