...
> 注：`scripts/` 内另有 `check-emotion-floor.js` / `check-hook-strength.js` / `check-typos.js`，为与 story-long-write / story-review 共享的字节级副本，本去味链路不使用，仅供一致性维护；改动任一共享脚本时须同步四目录副本。
