...
接纳与追踪闭环（`close`/`doctor` 通过）之后，用户说「本章已定稿，请核查」时按 `post-commit-audit.md` 对已定稿章做设计层一致性审计与双向裁决；`fix_design` 项经 `scripts/post_commit_audit.py project` 投影为修订计划后走 `workflow-revision.md` 门禁。审计不改变接纳状态，不重跑本协议的检查；报告与 ledger 落 `追踪/定稿核验/`。
