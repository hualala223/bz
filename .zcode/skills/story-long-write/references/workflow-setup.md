...
- 书名：{暂定名}
- 题材/类型：{主类型 + 副类型}
- 目标平台：{起点/番茄/晋江/其他}（= OB-3.6 `platform_profile`，双视图）
- 预计字数：{X} 万字
- 目标读者：{画像}（= OB-3.6 `audience_profile`，双视图）
- 读者契约：{本书承诺的核心阅读快感；参 reader-contract-and-progression.md}（= OB-3 `reader_contract`）
- 主角高光/代理权：{主角不可替代的判断、选择或贡献}（= OB-3 `protagonist_agency`）
- 核心期待债：{开篇必须偿还的承诺}（= OB-3 `core_debt`）
- 时间/地点/背景锚定：{起点时间+跨度+推进规则；核心地点+空间层级；时代社会背景}（= OB-3 `time_anchor`/`place_anchor`）
- 文风锚点：{3-5 关键词+一句话锚点语+句长标点基调}（= OB-3.5 `style_anchor`）
- 情感关系基调：{单/多/无 CP + 篇幅占比 + 与主线捆绑度}（= OB-3.7 `emotional_relations.meta`）
...
### 世界观骨架
- 时间/地点/背景：{起点时间·跨度·推进规则；核心地点·空间层级·移动路线；时代社会背景（与地点互证）}
- 核心设定：{区别于同类作品的独特设定}
- 力量体系：{如果有，简单概括}
- 社会结构：{影响故事的关键设定}
...
> **双轨对齐（OB 门 ↔ 本表单）**：本 Phase 2 表单是「快速模式」的一次性开书；交互开书（story-openbook）的 OB-3/3.5/3.6/3.7 四锚定门与本表单字段为**同一字段的双视图**——`目标平台/目标读者`=OB-3.6 `platform_profile`/`audience_profile`，`读者契约/主角代理权/核心期待债`=OB-3，`时间/地点/背景`=OB-3 `time_anchor`/`place_anchor`，`文风锚点`=OB-3.5 `style_anchor`，`情感关系基调`=OB-3.7 `emotional_relations.meta`。字段名以 `story-constraint-gate/references/merge-directive.md` 登记为准，两轨不产生第二权威。
...
**步骤 0：约束清单门（必须先执行）**：进入大纲搭建前，先读前序设定产物（`设定/`、`设定/_开书.json`、`选题决策.md`、`对标/`），按 `story-constraint-gate` 的规则扫描为 A 硬约束 / B 软约束 / M 元数据三表，**逐条向用户报告（硬阻断：不得概括）**，确认后落盘 `大纲/_约束清单.md`（缺失时先运行 `/story-constraint-gate` 生成）。已存在且 `设定/_开书.json` 的 `_meta.modifications` 未变则跳过重跑。

**体量门（卷纲前置）**：卷级大纲前，先按 `_开书.json` 的 `length_params`（total_chapters / words_per_chapter / total_words / volume_tier）确认全书总章节数、目标字数、四阶段章号区间、小中大高潮章号，并设**四张约束表确认位**：`character_chapter_map` / `arc_beat_map` / `worldview_conflict_schedule` / `theme_beat_schedule` 逐张展示摘要，逐张「确认/改/跳过该表」，确认后落盘 `大纲/_约束表.md`（A 硬约束遵循底线、B 软约束可协商）。
...
#### 四张约束表（`大纲/_约束表.md`，体量门确认后落盘）

四张表由 stepwise 体量门确认、落盘 `大纲/_约束表.md`，供细纲/场景卡/日更遵循；中途剧情追加只增不改既有行（同卷纲锁定规则）。

```markdown
## 约束表

### character_chapter_map（角色-章节出场映射）
| 角色 | 首次出场章 | 主要弧线章区间 | 关键转变章 | 备注 |
|---|---|---|---|---|

### character_arc_beat_map（弧线节拍映射，引用 G3 关键转变）
| 弧线/支线 | 章节区间 | 埋设章 | 引爆章 | 收束章 | 状态 |
|---|---|---|---|---|---|

### worldview_conflict_schedule（世界观冲突排期）
| 冲突 | 引入章 | 升级章 | 爆发章 | 收束/转承章 | 与终局底牌关系 |
|---|---|---|---|---|---|

### theme_beat_schedule（主题节拍排期）
| 主题单元 | 章节区间 | 承载情节 | 揭示/变奏章 | 备注 |
|---|---|---|---|---|
```
...
- 爽点蓄力链：{本卷蓄力/引爆的 TP-XX 行及进度（蓄力目标/当前进度），对照 `references/highlight-system.md`；stepwise 卷门按此展示爽点蓄力链}
...
#### 情节粗纲（卷纲之后、细纲之前，交 `story-outline`）

卷纲定稿后，做一次**情节粗纲**再进细纲。粗纲只定「每章要发生什么、为什么、受谁约束」的结构骨架，不展开 α点/五段式/情节点序列/爽点五字段（那些是细纲职责）。

- **执行体**：`story-outline`（独立 skill，源 B 第四组 G4.0→G4.10 方法 + 源 A markdown 轨道）。触发即读其 SKILL.md，不在此重复步骤。
- **产物**：`大纲/粗纲_第X卷.md`（逐章粗纲表）+ `大纲/粗纲总表.md`（约束表/结构/爽点预判/伏笔/节奏曲线五节全局视图）。不许落 `04_plot_outline.json`。
- **与四张约束表关系**：本文件「四张约束表（`大纲/_约束表.md`）」为体量门前置产出，story-outline 直接沿用、只做增量核对，不重复生成第二份。
- **接力**：粗纲交付后，本 Phase 3 的细纲流程消费 `大纲/粗纲_*.md` 的逐章 core_event + highlight_preset，展开为逐章细纲；确需改粗纲已确认的结构骨架，走跨产物修订门禁（`revision-governor` plan/verify），不静默改。

#### 细纲（已迁移至 `story-outline-detail`）

> ⚠️ **执行权迁移**：细纲主体（细纲自适应参数、细纲模板、细纲节奏检查、收变开留存、剧情批滚动、脚本链）已独立到 `story-outline-detail` skill（消费第五组 G5.x + 接管粗纲让渡字段）。**下文的细纲小节为存档/对照，不再作为细纲执行入口**——出细纲时读 `story-outline-detail/SKILL.md`。本文件 Phase 3 只保留「体量门 → 卷纲 → 粗纲接力 → 细纲接力」的编排。

#### 细纲自适应参数（G5.0，细纲前置·存档）

按 `_开书.json` 的 `length_params.total_chapters`（C）决定细纲详细程度档：

| 全书章数 C | 细纲详细程度 |
|---|---|
| C ≤ 50 | 全部章节走完整细纲模板（下节） |
| 51 ≤ C ≤ 120 | 关键章（转折/高潮/单元首尾/伏笔引爆）走完整模板；普通推进章走「最小字段集」档（核心事件/α点单行/爽点/章节定位/结尾钩子，其余字段可略） |
| C > 120 | 仅转折点/单元首尾章走完整模板，其余合并为一行速览 + α点单行确认（对齐 stepwise 纲门「最小字段集」档合并规则） |

细纲事件素材数按本章 `S=max(2,min(5,round(字数目标/800)))` 先定 S 再排情节点；S 不足时从 8 类补充事件库（主线核心→支线→配角事件）补足，不为凑数堆无效冲突。节奏检查粒度 `G_outline=max(2,min(10,floor(C/30)))`（每批需覆盖的核心节拍数上限，供批末核对）。
...
- α点（目标→障碍→代价 + 情绪弧线）：{本章戏剧性核心：主角目标（≥15字）→ 关键障碍 → 承担代价；情绪弧线 起点→终点（承接下方「目标情绪」前后状态）。scene-card 按此 α点继承章级「目标情绪」，场景内微调不得偏离本章整体情绪方向}
...
- 爽点五字段（角色/类型/级别/蓄力目标/蓄力进度）：{本章若设显性爽点则逐字段填；蓄力链与类型见 `references/highlight-system.md`（九种类型/三段式/密度标准/TP-01~08）；无显性爽点章可写“无”，不硬编}
...
- 事件素材数校验：{本章建议事件数 `S=max(2,min(5,round(字数目标/800)))`，直接消费 `_开书.json` length_params.words_per_chapter；实际铺陈事件数低于 S 时，按「补充事件库」补足——先写主线核心事件，S 不足再补支线/配角事件，不为凑数堆无效冲突}
...
#### 结尾设定和钩子
- chapter_question：{本章留给读者的核心疑问（一句话），与上一章钩子承接、为下一章铺路；双向追溯}
- hook_chain_status：{钩子链状态：承接自 TP-XX / 新设钩子 / 本章兑现；与卷纲爽点蓄力链对齐}
...
**理想紧张值**（高压推进章）：{0-20 铺垫蓄势 → 20-60 冲突升级 → 60-85 引爆/兑现 → 85-100 收束余韵}；**7 项基础检查**：① 开头是否给读者一个阅读理由（钩子/悬念/待解问题）；② 是否有一次明确的目标推进或关键选择；③ 章内情绪是否有起伏（至少一个峰与一个谷，不含无波平推）；④ 章尾是否落在未解决的问题或推进性余势上（不是平顺收尾）；⑤ 若设爽点，是否完成「蓄力→引爆→余韵」闭环而非硬造高潮；⑥ 高压章是否给出压力来源而非纯情绪堆叠；⑦ 与上一章/下一章钩子是否真实承接。**4 项豁免**：纯过渡章、纯信息整理章、低压生活章、开篇背景章可豁免②⑤，但豁免章仍须满足①④。**爽点-紧张值联动阈值**（引爆章）：minor ≥5 / medium ≥7 / major ≥9 / 高潮=10 且全篇唯一 TP-08。任一检查不满足，先在细纲阶段调整本章结构或钩子，不留到正文阶段才返工。
