...
> **位置（横跨多阶段）**：本 skill 覆盖全书流程的 阶段 0 选题、阶段 5 大纲、阶段 6 正文（见 `story-stepwise/references/book-roadmap.md`）；其中的开书（阶段 1）由 `story-openbook` 承接，人物（阶段 4）由 `story-character-lab` 承接。快速模式下开书走本 skill Phase 1-2 一次性表单，字段与 OB 门双轨对齐（见 `references/workflow-setup.md`）。
...
| **定稿核验** | "本章已定稿，请核查" / "定稿核验 第N章"（第 N 章接纳与追踪闭环完成后） | 加载 `references/post-commit-audit.md`：对已定稿章做设计层一致性审计（细纲/人设/卷纲边界/伏笔/规则 vs 正文），产出 `追踪/定稿核验/第NNN章_核验.md` 出入清单，S1/S2 逐条交用户双向裁决（照设计改正文 / 照正文改设计）；fix_design 项用 `scripts/post_commit_audit.py project` 投影为修订计划后并入大修闭环。只读已提交产物 + 写报告，不自动修改正文/大纲/设定 |
...
**匹配优先级**：同时命中多行时，按 大修 → 定稿核验 → 写指定章 → 分支推演 → 补纲/扩纲 → 日更续写 → 开书 的顺序匹配。用户明确要求比较多个走向时必须先停在推演结果，不得把任一方案自动视为已选；用户点名要"细纲/补纲/规划剧情"而未要正文时，优先入 补纲/扩纲，不入日更。日更续写的 AND 条件（项目已有正文+追踪）不满足时，提示用户"项目还没有正文，建议先开书/写第1章"。「本章已定稿，请核查」只入定稿核验，不解释为日更或大修触发词。
...
产出全书体量与阶段总览、卷级大纲、**情节粗纲**、逐章细纲；含可选分支推演、大纲安全七检、大纲安全审查、分批建纲、「收一个、变一个、开一个」留存硬门与「中途补纲/扩纲小流程」。

**三步骤分离**：大纲搭建设中「卷级大纲 → 情节粗纲 → 逐章细纲」三段。卷纲定稿后先做**情节粗纲**，由独立 skill `story-outline` 承接（产 `大纲/粗纲_第X卷.md` + `大纲/粗纲总表.md`）；再据粗纲展开**逐章细纲**，由独立 skill `story-outline-detail` 承接（产 `大纲/细纲_第NNN章.md`，消费第五组 G5.x）。本 Phase 3 只保留「体量门 + 卷纲 + 三段接力编排」，粗纲/细纲的执行主体在各自独立 skill。
...
| 追踪/冷读/{run-id}/ | 范围审查 | 卷末、大修后或关键发布前 | 按 `sequential-cold-read.md` 顺序推进；S1/S2 未清不得开新卷。卷末如需更广维度的整体体检，可再调用 `/story-final-check`（冷读 issue 可并入其综合修改清单，见 final-check 步骤 0 衔接说明） |
...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - **人物言行锚定**（本章每个人物的言行举止符合人物设计，参考 `references/character-voice-anchor.md`）：按细纲出场名单，逐个读 `设定/角色/{名}.md` 的语言特征卡 / 认知信念 / 行为字段 + `追踪/角色状态/{名}.md` 当前状态，产出「人物言行锚定卡」（语言锚 + 思维认知锚 + 行为锚 + 状态锚）。这张卡进入 narrative-writer 的 prompt 作硬约束：本章该人物的台词、内心、认知描写、动作不得跳出锚定卡；细纲要求与人设冲突时标「言行与设计冲突」回细纲/卷纲确认。人设卡缺可执行样本时按性格类型映射 `行为词典/` 对应词典补足（只检索、不改人设），人设充分则不查词典
   - **模块召回、题材卡与文风召回**：
...
8. **字数验证**（写作完成后的第一件事）：**字数口径＝纯字（汉字数，不含标点与其他字符，见「字数验收权威」）**。不要用 `wc -c`、含标点的去空白字符统计或模型估算；唯一权威读数＝同轮运行的 `"$PYBIN" scripts/prose_metrics.py <候选正文>` 输出的 `character_count`（汉字计数）。Windows 不直接假定 `python3` 命令可用，探测顺序 `python3/python/py`。
	- 同轮运行 `"$PYBIN" scripts/prose_metrics.py <候选正文>`，把汉字数与短/中/长句占比、平均/中位句长、段落均长与句段比作为**唯一实测值**写入候选 Gate 报告；这些统计只用于定位读感复核，不设跨题材配额。narrative-writer 的口头估算不得替代脚本结果。
   - 汉字数 < 细纲目标 90%：对照情节点预算找欠账点。密点（爽点/打脸/反转）被写薄时，重写到对应预算；低压/关系/信息整理章则补细纲内已有铺垫、互动或表演节拍，不硬塞爽点。若现有细纲没有足够可展开内容，停止并输出 `outline_underfilled` 欠账点，先补纲/确认，不能让正文自造新剧情。
   - 汉字数 > 章目标×1.1：压过场、合并疏点、删多余过渡，不删主线爽点凑数。
	- 90% 只是放行下限，目标仍是 `[章目标, 章目标×1.1]`（均按纯字口径）；重写后重新统计，落进区间再进入步骤 9。
...
12. **候选 Gate、接纳与追踪**：先运行 `scripts/chapter_candidate.py check`；该命令会自动实测句段分布、核对本章细纲连续照搬，在画像可用时加入已接纳/黄金声线双向漂移 advisory，并生成近六章结构指纹冷读卡。主 Agent 必须回答结构五问并把具体证据纳入候选报告；相似度本身不阻断接纳，也不授权自动改文。**候选停靠报告＝章后核查报告**：报告必须内含 `references/chapter-review-loop.md` 第一段九项核验（去AI味/语言节奏/错别字禁用词/人物言行符合人设/风格一致性/腔调一致性/距离调节合理性/情节链接/伏笔钩子）的逐项结论与问题清单（`blocking` 已当轮清零，`major/minor/advisory` 交付用户处置），以及第二段上下文衔接核查的「✅/⚠️」结论——不另设第二次停靠，用户对清单的处置（确认修改项或直接接受）与候选接纳在同一停靠点完成。`review` 模式向用户报告候选标题、字数、关键变化、Gate 结果、九项清单、衔接结论和路径后停止；用户明确接纳后，或本次任务已有有效 `auto` 授权时，才依次执行 `approve --confirm ACCEPT` 与 `promote --confirm PROMOTE`。正式正文写入后，按 workflow-daily 构造本章唯一追踪事务并执行 `scripts/tracking_commit.py commit`；成功复检后依次运行 `chapter_candidate.py close`、`voice_profile.py update --project`（未配置画像时安全跳过）和 `story_doctor.py --project`。任一步失败都保留候选/事务现场，不手改派生文件、不写下一章。**接纳闭环全部通过后，默认自动执行定稿核验**（`references/post-commit-audit.md`，主线程 solo，不依赖触发词）：对已定稿章做设计层一致性审计并落 `追踪/定稿核验/第{N}章_核验.md`；`verdict=clean` 时在本章收尾报告引用核对结论，本章结束；`verdict=conflicts` 时停下列出 S1/S2 交用户双向裁决（fix_text/fix_design），裁决未闭环前不得写下一章。仅当项目 AGENTS.md 明确声明「定稿核验改为手动触发」时，才降级为在收尾报告中提示一次触发词。不能先把候选吸收进画像再检查自己；黄金集合也不得自动吸收新章。本章首次引入会复用的具名角色/势力时，仍按 `references/workflow-setup.md` Phase 3 规则补建静态 `设定/` 档案。
...
**字数口径（全书统一，硬定义）**：所有「字数」——章字数目标、细纲/场景卡字数预算、正文字数验收、`length_params.words_per_chapter`、`total_words`——一律指**纯字＝汉字数，不含标点符号与其他任何字符**。唯一量具＝`scripts/prose_metrics.py` 输出的 `character_count`（汉字计数，等价于 `voice_profile.chinese_count`）；平台后台显示字数（通常含标点）只作参考，不作为验收口径。

长篇每章只按本章细纲的 `字数目标` 与步骤 8 的统一 90% 放行下限验收。节奏类型只决定情节点疏密和展开方式，不再叠加另一套静态最低字数。

**细纲缺 `字数目标` 时**：从 `设定/_开书.json` 的 `length_params.words_per_chapter` 取值，走同一条 90% 放行下限，并提示补纲；`length_params` 也缺失时，按 AGENTS.md 篇幅字段权威**向用户索要**，不得臆造默认值代入公式。
...
**确定性收尾**：错别字校验通过后，主会话对实际落盘文件运行 `node scripts/check-ai-patterns.js --check --fail-on=blocking 正文/第XXX章_*.md`。blocking 命中先回正文改写并复扫；advisory 逐条读原文判断，确属问题才改，功能性写法标 `[需复核]`。其中 `formulaic-parallelism` 必须连同对话一起复核，不能因为 hook 不阻断台词就略过。
...
#### 单章语言与节奏精修（第七/八组单章级核验+修改，硬性必须，非可选）

去 AI 味审查查的是「不该有的东西」（删冗余/解释腔/上帝感），做完后还要补一轮「该有却缺席/该精修」的**语言与节奏精修**（对称于 check-ai-patterns ↔ check-emotion-floor）：按 `references/chapter-language-revision.md` 对本草依次核验并修改六项——①关键场景句子节奏（高张力句长 8-15 字、动词密度 ≥40%，以 prose_metrics 实测为准）②标点服务情绪（标点服从语气，破折号/感叹号密度不失控，消灭标点与情绪矛盾）③语言综合精修（动词强度/关键场景三感官以上锚点/比喻分级与密度，正向下限为主）④海明威修改法（deslop 未显式覆盖的四项：被动改主动/分号削减/叙述段感叹号/限定词大扫除）⑤肢体语言终检（无孤立抽象情绪词、姿态簇一致、撒谎场景≥2 说谎线索、查点头微笑叹气高频重复）⑥读者三问终检（那又怎样/噢是吗/呃——对本章关键节点自问）。改动只碰「怎么说」不碰剧情/伏笔/人设，改后复跑确定性脚本清零。低压过场章可只命中 1-2 项，关键章六项全过。此处无需单独 report，结果并入「检查后更新 `_质检进度.md`」（如有「语言节奏精修」列打勾）。
...
本节前面的检查是「写完当轮清零」的机械化部分。除此之外，每章写后还必须走一遍**统一交互闭环**（`references/chapter-review-loop.md`），把「先列问题清单 → 用户确认 → 修改 → 用户手改 → 手改提炼下游联动 → 上下文衔接重点核查 → 确认结束」串起来。触发：**默认随候选停靠报告自动进入**——第一段九项清单并入候选报告交付（Phase 4 步骤 12），不另设停靠；「核查本章」「对该章内容进行核查和修改」「检查这章」为随时可用的手动触发词（含对已定稿章补跑）。关键差异与新增：

- **先列清单、等确认再改**：第一段自动核验九项（去AI味 / 语言节奏 / 错别字禁用词 / 人物言行符合人设 / 风格一致性 / 腔调一致性 / 距离调节合理性 / 情节链接 / 伏笔钩子）产出分级问题清单，停下等用户确认要改哪些；`blocking`（语言门/HTML/截断/退化）例外，当轮清。
- **手改提炼是强制步骤**：用户手改正文后，先提炼关键改动点（diff），分为「纯文风/措辞」与「剧情/事实级」；后者对照 `revision-impact-and-canon.md` 影响关系清单，判是否波及后面未写的细纲/场景卡——波及则提醒并按门禁联动修改下游设计（改前经用户确认），并走 `mode=revision` 追踪同步。
- **声线样本提示**：手改较多或作者满意时，提示是否纳为黄金声线样本（`voice_profile.py golden-build`），让后续写作模仿作者风格（`accepted-voice-profile.md`「黄金声线样本」）。
- **第二段上下文衔接核查**：手改+联动后，重点核查伏笔/钩子/节奏/人物言行跨章/距离走向的跨章衔接，出「✅/⚠️」结论，确认后本章才结束。
...
| 人物言行锚定（言行符合人物设计） | `references/character-voice-anchor.md` |
...
| 定稿核验（设计层一致性审计与双向裁决） | `references/post-commit-audit.md` + `scripts/post_commit_audit.py` |
...
| 单章语言与节奏精修（第七/八组单章级核验+修改） | `references/chapter-language-revision.md` |
| 章后核查与修改闭环（统一交互封装） | `references/chapter-review-loop.md` |
