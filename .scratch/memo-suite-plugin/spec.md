...
### 复习域单一入口合并（ticket 168 重构，用户 grill 三轮拍板）

> 本节覆盖上方 36-44b 与下方「命令 id 全清单」复习段、「设置项总表」复习段（含移动端全屏域清单）：凡涉复习计划面板、开始复习、加入复习计划、移出复习计划、跳转逾期、选择难度、快捷标记 ×4 的条目随 ticket 168 **退役**；复习收敛为单一入口「复习（按数量）」+ 合并能力。详细 spec 见 `issues/168-review-single-count-entry.md`，决策见 ADR-0077。

- **命令**：复习域仅保留 `bz-review-count`（复习（按数量））；总命令 39→29；旧命令 id 外部调用失效（用户接受）。
- **合并能力**：篇数弹窗 ⚙️ →「复习设置」弹窗（标题更名）；新增「复习条目管理」组（列出全部条目、挂起标灰、逐条移出 + toast 撤销恢复）；做题家子项（允许多选题/每篇出题数量/打乱顺序/出题难度）改为**常显**（「用做题测难度」forceQuizForReview 退役，键与行删除，旧值忽略）。
- **逾期监控**：60s 轮询与 enableAutoNotify 门控保留；常驻通知「去复习」→ **全 vault 逾期按数量会话**（任意目录、篇数=min(逾期数, 每日复习上限)，截断提示剩余；选择逻辑为 count 模块纯函数）。
- **保留不动**：监听文件夹自动收编/删除确认/改名跟径/排除名单、文件树标记（reviewTreeBadge）、按数量复习既有流程（篇数弹窗/安排算法/做题循环/汇总页/重做本篇/当天已复习排除）——冻结。
- **删除**：复习计划面板（列表/卡片/抽屉/搜索/归档/难度弹窗/ESC 层）、普通复习（跳转手动评级）路径、`reviewMobileDefaultFullscreen` 键与设置行（随面板退役，旧值忽略）。
- **数据**：review.json 零格式改动，旧数据直接可读，继续参与逾期/阶段采样；复习全面做题化（AI 未配置明确提示）。
