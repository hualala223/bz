...
/** 惰性 schema 加载器（与各域 ⚙️ 弹窗同源）。本地口径：域集 = 本仓库现有 20 域 +
 *  上游并入新域；通用/AI 组从本地 mainSettingsSchema 按组名拆分（本地无独立导出）。 */
const schemaLoaders: Record<string, () => Promise<SettingsSchema>> = {
  // 通用组：本地主设置页的「数据存储路径」组 +「数据体检」按钮行（D4：检查项直达体检面板；
 ...
 *  本地口径：待办/剪藏本/影院/书库等为上游线域名，此处映射本地对应域（备忘录/剪藏/影视/书库）。
...
export const DOMAINS: DomainDef[] = [
  ...
  { id: 'clipping', name: '剪藏本', icon: DOMAIN_ICONS.clipping, desc: '未读流与剪藏笔记一体化', schemaLoader: schemaLoaders.clipping },
