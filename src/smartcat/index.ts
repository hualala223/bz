...
export async function ensureSmartCat(app: App, opts?: { startHidden?: boolean }): Promise<void> {
  ...
  // 知识盒（ADR-0066/0072）：knowledge 域经 'knowledge:tasks' 通道派发 → 行为流（ticket 136：视频 converted + 术语 term-generated）
  busUnsubs.push(onDomainEvent<KnowledgeActionEvent>('knowledge:tasks', (evt) => notifyKnowledgeAction(evt)));
...
// ------------- 知识盒动作观察（ADR-0066/0072：knowledge:tasks 域事件接入小橘行为流；ticket 136 收敛为两节点） -------------

/** 知识盒动作观察处理（knowledge 域经 emitDomainEvent('knowledge:tasks', evt) 派发 → 总线订阅进入）。
 *  未初始化 / 未启用（noteSource 关）→ 静默。
 *  视频转文献成功（converted）/ 术语生成成功（term-generated）→ 行为流新建条目
 *  （ticket 136 用户拍板只收这两类；添加任务/解析/编辑/失败跳过）。 */
function notifyKnowledgeAction(evt: KnowledgeActionEvent): void {
  if (!initialized || !memorySystem || !data?.config?.noteSource) return;
  const structured = buildKnowledgeStructured(evt);
  if (!structured) return;
  // 同事件同 key 近 300ms 防重（双击保存等双入口场景）
  if (notifyDeduped(structured.action, knowledgeActionKey(evt))) return;
