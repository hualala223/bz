/**
 * 知识盒动作观察（knowledge 域 → 小橘行为流；ADR-0066 起建、ADR-0067 语义充实、ADR-0072 迁出为新域）
 * knowledge 域（视频批处理引擎 / 术语生成流程）经 emitDomainEvent('knowledge:tasks', evt) 派发 → 总线订阅进入，
...
/** 知识盒动作事件（knowledge 域 emitDomainEvent('knowledge:tasks', evt) 载荷；ADR-0066/0072） */
...
/** 知识盒事件 → StructuredMeta（converted/term-generated 之外返回 null——ticket 136 用户拍板只收这两类） */
