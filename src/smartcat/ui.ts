...
export function smartcatSettingsSchema(opts: {
  ...
  onPowerStateChange?: (enabled: boolean, offMode: string) => void | Promise<void>;
}): SettingsSchema {
  ...
  // ===== 小橘电源组（ticket 103 本地移植：启用开关 + 三档关闭方式；主设置页 schema 化后迁居此处）=====
  const powerGroup: GroupDecl = {
    icon: 'power',
    name: '电源',
    rows: [
      {
        ...
        binding: bindBehavior('smartcatEnabled'),
        onChange: (v, ctx) => {
          ctx.refreshVisibility();
          void opts.onPowerStateChange?.(v, (getSettings() as any).smartcatOffMode);
        },
      },
      {
        type: 'select',
        ...
        binding: bindBehavior('smartcatOffMode'),
        options: [
        ...
        onChange: (v, ctx) => {
          ctx.refreshVisibility();
          if (!(getSettings() as any).smartcatEnabled) void opts.onPowerStateChange?.(false, v);
        },
      },
    ],
  };
