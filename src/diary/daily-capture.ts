 ...
 * 与每日触动点记录（`# 当日触动点`，票 302 前为 `# 当日复盘`）的落盘层（daily-write.ts）
 * 复用同一套定位逻辑，只用三个入口切差异：
...
/**
 * 把**多行块**插入指定小节末尾（纯函数；写日记 / 每日触动点记录落盘用，ADR-0114）：
 ...
 *    （placed = 'created'，触动点记录用：`# 当日触动点` 不在骨架里）；为假则整块追加文末
...
/**
 * 多行块写入日记小节（写日记 / 每日触动点记录薄壳，ADR-0114）。
 * 与单行捕获共用写壳；`createIfNotFound` 决定未命中时是否在文末现场建节
 * （触动点记录 `# 当日触动点` 不在骨架里 → 建；写日记 `# 随笔` 由骨架自带 → 不建，
 * 追加文末并提示）。旧落点 `# 当日复盘` 不做别名兼容（票 302 二次裁定），历史小节不动。
 */
