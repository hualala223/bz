...
/**
 * 主页磁贴自动播种（本地 launcher 域入口页接线，保持本地行为）：缺 bz-attach-move 磁贴时
 * placeAtEnd 追加到末尾，保存 launcher.json（幂等，失败静默）。
 */
export async function ensureAttachSeed(app: any): Promise<void> {
  try {
    ...
    for (const key of ['desktop', 'mobile'] as const) {
      ...
      if (tiles.some((t: any) => t.commandId === ATTACH_COMMAND_ID)) continue;
