 ...
 *
 * memo.json 是备忘录域的唯一条目数据文件（上游线曾由 memo/todo 两域同源共读，
 * 本地仓库无 todo 域，故为单视角结构核对）。本检查对磁盘原文做结构核对：
 * - 口径快照归一（memo/data.ts DataManager.loadItems 的映射快照，只取归一、不带其补 id 写回副作用）；
...
/** 单视角计数：条数 + 完成数（memo 口径 = completed 非 null） */
interface ViewCount {
  total: number;
  done: number;
}
...
export function consistencyIssuesOf(stats: MemoConsistencyStats): { summary: string; issues: CheckIssue[] } {
  ...
  if (stats.total < 0) {
    return {
      ...
      issues: [
        {
          severity: 'error',
          title: 'memo.json 不是条目数组形态（备忘录无法读取）',
          detail: '读取链期望「条目数组」；当前文件是其他形态，请从 CONFIG/.CORRUPT/ 留档或备份恢复。',
        },
      ],
    };
  }
  if (stats.nonObject > 0) {
    issues.push({
      severity: 'error',
      title: `${stats.nonObject} 条非对象条目（备忘录的读取会在这里中断）`,
      detail: '数组里混入了非对象内容（字符串/数字等），加载会失败，请从留档或备份修复。',
    });
  }
  if (stats.duplicateId > 0) {
    issues.push({
      ...
      detail: '同 id 条目会被当成同一条处理：勾选完成一条，另一条也显示完成。',
    });
  }
  ...
  const summary = issues.some((i) => i.severity === 'error')
    ? '发现结构异常'
    : bad > 0
      ? `条数 ${stats.total} · 完成 ${stats.memoView.done}，另有 ${bad} 处小问题`
      : `条数 ${stats.total} · 完成 ${stats.memoView.done}，字段形态与约定一致`;
  return { summary, issues };
}

/** 检查四：memo.json 结构一致性（只读） */
