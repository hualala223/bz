 ...
 *
 * memo.json 是备忘录域的唯一条目数据文件。本检查对磁盘原文做双链核对：
 * - 单例链：todo/data.ts loadItems 的归一口径快照（completed: item.completed || null）；
 * - 直读链：todo/data.ts 的 normalizeItem 实际归一（同一约定字段集）；
