/**
 * 知识盒数据管理器（视频转文献，knowledge 域；ADR-0066 正名「知识盒」，ADR-0072 迁出为新域）
 * knowledge.json 读写（jsonStore）、任务 CRUD、状态流转、时间格式校验。
 * D3 可靠写契约原语 1 收编：全部「读→改→写」事务整体入 core per-path 串行队列
 * （enqueueFileTask，键 = knowledge.json 路径）——下载守护进程回写状态与面板增删任务并发时
...
import { cleanUrlText, extractUrlFromText, normalizeSourceUrl } from './source';
 ...
 *  ticket 284：先抠出文本里的链接——手机分享文本 `【标题】 https://…` 整段粘进来时，净化不再空转。 */
export function normalizeUrl(raw: string): string {
  return normalizeSourceUrl(cleanUrlText(extractUrlFromText(raw)));
}
...
export const KnowledgeData = {
  ...
  /** 票 290 一次性迁移守卫（literature.json → knowledge.json，只复制不改写，旧文件保留在原处） */
  ...
  /** 一次性迁移（票 290，照上游 ADR-0112 范式）：knowledge.json 不存在而 literature.json 存在时原样复制一份（任务历史零丢失） */
  ...
  async loadTasks(): Promise<KnowledgeTask[]> {
    // 旧数据文件一次性迁移先行（literature.json → knowledge.json，票 290）
