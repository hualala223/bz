...
describe('launcher 数据层', () => {
  ...
  describe('load/save launcher.json', () => {
    ...
    it('解析失败 → 空布局且原文件留档到 CONFIG/.CORRUPT/（不再静默覆盖）', async () => {
      const vault = new MockVault();
      const broken = '{broken';
      ...
      // 统一读写语义：原内容集中留档（CONFIG/.CORRUPT/<名>.<时间戳>.bak，D1 契约），原路径重建空配置
      const base = LAUNCHER_PATH.split('/').pop() || LAUNCHER_PATH;
      const backups = [...vault.files.keys()].filter((p) => p.startsWith('CONFIG/.CORRUPT/' + base + '.') && p.endsWith('.bak'));
