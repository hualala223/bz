...
/** memo 条目全字段样本（16 字段齐全 = 无缺失漂移；票 298 增 category/urgency） */
function fullMemoItem(over: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    id: 'item-1', title: '买牛奶', scene: '生活', priority: 'minor',
    category: 'must', urgency: 'not', created: '2026-01-01 00:00:00',
...
function makeApp(
  ...
  setApp(app as never);
  // 本地 library/movie 数据源经设置派生目录（getSettings 未注入会抛）——注入最小提供者
  setSettingsProvider(() => ({ libraryFolderPath: '书库', bookTag: 'book' }) as never);
...
describe('检查二：字段漂移', () => {
  ...
  it('漂移字段常量与域 normalize 契约一致（16/15/3；票 298 memo 增 category/urgency）', () => {
