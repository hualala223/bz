...
describe('UIManager 解锁弹窗', () => {
  ...
  it('首次无清单：标题「设置主密码」+ 再次确认 + 硬警告；未勾选风险确认拒绝设置，勾选后成功', async () => {
    ...
    expect(dialog.querySelector('.bz-encrypt-dialog-ack') || dialog.querySelector('.bz-lockscreen-ack')).toBeTruthy(); // 硬警告确认勾选（仅首设显示）
    const confirmBtn = findConfirmBtn(dialog);
    // 两次不一致
    (inputs[0] as HTMLInputElement).value = 'pw12';
    (inputs[1] as HTMLInputElement).value = 'pw22';
    confirmBtn.click();
    ...
    expect(dm.unlocked).toBe(false);
    // 勾选后再确认 → 设置成功
    const ackBox = findAckInput(dialog) as HTMLInputElement;
    ackBox.click();
    confirmBtn.click();
    await p;
  ...
  it('已有清单：解锁屏标题「保险库已上锁」，错误密码失败、正确成功', async () => {
...
describe('EncryptAppController', () => {
  ...
  it('openManager：未解锁先弹解锁；解锁后显示面板', async () => {
    ...
    const inputs = dialog.querySelectorAll('input[type="password"]');
    const confirmBtn = findConfirmBtn(dialog);
    (inputs[0] as HTMLInputElement).value = 'pw1234';
    confirmBtn.click();
    (inputs[1] as HTMLInputElement).value = 'pw1234';
    (findAckInput(dialog) as HTMLInputElement).click();
    confirmBtn.click();
    await p;
...
describe('解锁弹窗：清单损坏重设确认 + 首设写失败（雷 1/4 UI 侧）', () => {
  ...
  it('连续失败递增冷却：1s/2s/4s 封顶 8s；冷却期内拒绝尝试；成功后复位（P2）', async () => {
    ...
    const confirmBtn2 = findConfirmBtn(dialog2);
  ...
  it('解锁弹窗：打开即自动聚焦密码输入框（移动端直接弹键盘）', async () => {
    ...
    // 取消关闭
    // 新解锁屏无「取消」按钮：点遮罩即取消（ADR-0124 决策 5）
  ...
  it('解锁弹窗：点击遮罩（内容区外）关闭面板 = 取消；点内容区不关闭', async () => {
    ...
    // 点内容区（box）不关闭
    const box = mask.querySelector('.bz-lockscreen-box') as HTMLElement;
