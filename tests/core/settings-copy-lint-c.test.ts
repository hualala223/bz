...
const WHITELIST = new Set<string>([
  // secondbrain 本机局域网 IP：desc 为探测到的 IP/接口列表（ticket 122 自查信息本体，符号不可避免；票 273 随上游 panel schema 吸收）
  ...
  // 2026-09-13（票 273）：「启用」行已随上游 panel schema 退役（secondBrainEnabled 仅剩启动自动加载门），豁免一并删除
]);
