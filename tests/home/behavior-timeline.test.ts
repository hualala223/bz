...
describe('behaviorSourceDomain（行为流 source → 首页域 id）', () => {
  it('已盘点来源逐一归口（渲染徽标/彩点 hasEvent 认这个 id；目标 id 走本地命名，票 288）', () => {
...
describe('mapBehaviorEvent（ADR-0132 映射表）', () => {
  ...
  it('待办：added 保留「新增待办」前缀（todoCreated 派生契约）、completed 产出', () => {
    const added = mapBehaviorEvent(item('memo', 'added', '买牛奶'));
    expect(added).toMatchObject({ domain: 'todo', kind: 'progress', text: '新增待办『买牛奶』' });
    expect(added?.text.startsWith('新增待办')).toBe(true);
...
describe('behaviorToDays（按本地日分桶）', () => {
  it('今天起往回 N 天、事件升序、窗外与无效动作剔除', () => {
    ...
    expect(days[0].events.map((e) => e.text)).toEqual(['新增待办『早』', '标记《晚》已看']);
