...
function riverData(counts: Partial<typeof EMPTY_COUNTS> = {}, streak?: RiverStreak): RiverData {
  const day = (i: number) => ({
    dateStr: ds(i),
    events: [],
    summary: { diary: 0, movies: 0, books: 0, todoDone: 0, todoCreated: 0, pomodoros: 0, pomodoroMinutes: 0 },
    firstTs: null,
  });
  ...
  return {
    ...
    collectRecent: [], // issue 246：收集快照入 RiverData 后的必填字段（本文件不涉彩点/快照，恒空）
...
describe('DOMAIN_MENU 形状', () => {
  ...
  it('2026-09-11 新增项（本地映射版，票 288）：命令 id 与开关标记（danger / keepHome / dynamic）', () => {
    ...
    // 上游有而本地无命令面的项不挂（票 288）：bz-memo-note-binding / bz-bookshelf-continue /
    // bz-knowledge-note-video / vault 组（本地密码本并入保险库，ADR-0085）
