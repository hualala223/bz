...
/**
 * 日程三段（ADR-0123：它是 `buildPlanContent()` / 模板里 `# 日程规划` 之下的那段；
 * 空占位三个 `- [ ] ` 已按用户裁定删除，待办行直接写在 `## 代办事项` 下）。
 */
const PLAN_BODY = [
  '## 代办事项',
  '',
...
/** vault 模板真源里的骨架部分（模板文件 = 骨架 + 空行 + 日程三段，ADR-0123） */
...
/** vault 模板文件当前内容（骨架 + 日程三段；ADR-0123 起三段由模板自带、空占位已删），末尾无换行 */
const TEMPLATE_FILE = [...TEMPLATE_SKELETON, '', PLAN_BODY].join('\n');
