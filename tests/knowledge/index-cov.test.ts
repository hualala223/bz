/**
 * 知识盒补充覆盖测试（src/knowledge/index.ts 未触达分支）：
 ...
 * ticket 136 改版：入口打开的是「视频录入」面板（任务队列 + 叠开添加弹窗），id 前缀改 knowledge-/lit-；
...
const noteGen = vi.hoisted(() => ({
  ...
  generateTermNote: vi.fn().mockResolvedValue('知识盒/AI 简介.md'),
...
describe('openKnowledgeAddTask（聚合讯「保存至文献」入口，ticket 134/ADR-0068）', () => {
  ...
  it('ensure 幂等初始化 → 视频录入面板叠开添加弹窗，预填链接/标题/UP主（新增模式标题）', async () => {
    ...
    setApp(app);
    setSettingsProvider(() => ({ storagePath: 'CONFIG/STORAGE', knowledgeDirectory: '知识盒' }) as any);
