/**
 * 知识盒批量处理器测试（src/knowledge/processor.ts，ticket 136/ADR-0071：AI 回迁插件侧）：
...
const noteGenMocks = vi.hoisted(() => ({
  generateVideoNote: vi.fn(async () => '知识盒/测试.md'),
}));
...
describe('BatchRunner', () => {
  ...
  beforeEach(() => {
    ...
    noteGenMocks.generateVideoNote.mockReset().mockResolvedValue('知识盒/测试.md');
    clearNotices();
  });
  ...
  it('非桌面端（无 window.require）→ 错误提示且不进入处理', async () => {
    ...
    expect(getNoticeMessages().join('\n')).toContain('仅桌面端可用：知识盒处理需要 Node.js 外部进程');
  ...
  it('严格串行 + close(0) 驱动插件侧 AI：读转录 → generateVideoNote → 删临时文件 → 成功落库归档 + converted 事件', async () => {
    ...
    try {
      ...
      expect(first.notePath).toBe('知识盒/测试.md'); // generateVideoNote 返回的笔记路径
      ...
      expect(converted[0]).toMatchObject({ kind: 'converted', id: tasks[0].id, url: 'BV1xx411c7mD', notePath: '知识盒/测试.md' });
    } finally {
      sub();
    }
  });
  ...
  it('[bz-result] video 为 null → videoPath null（keepVideo 关闭场景）', async () => {
    ...
    expect(all[0].notePath).toBe('知识盒/测试.md');
  });
  ...
  it('[bz-info] 解析信息：落库 title/uploader + onTaskInfo 回调，不发射知识盒域事件（P3-2，契约收敛 converted/failed）', async () => {
    const bus: any[] = [];
    const sub = onDomainEvent('knowledge:tasks', (e) => bus.push(e));
    try {
      const tasks = await seedTasks();
  ...
  it('[bz-info] 只补空（上游 issue 278）：task.title 已有值不被下载阶段覆盖、UP主 空则照补', async () => {
