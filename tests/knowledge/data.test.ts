...
/**
 * 知识盒数据层测试（视频转文献，knowledge 域；ADR-0066 正名，ADR-0072 迁出）：
...
describe('isTerminal / normalizeUrl', () => {
  ...
  it('normalizeUrl 带参链接剥追踪参数（上游 issue 278：净化单源 normalizeSourceUrl），保留内容参数 p/t', () => {
...
describe('KnowledgeData（knowledge.json）', () => {
  ...
  it('updateTask 保留 id 与 created，仅合并补丁', async () => {
    ...
    await KnowledgeData.updateTask(t.id, { status: 'success', notePath: '知识盒/测试.md' } as any);
    const all = await KnowledgeData.loadTasks();
    expect(all[0].id).toBe(t.id);
