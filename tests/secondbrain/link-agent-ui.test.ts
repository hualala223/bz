 ...
 * - 文献笔记生成即跑（issue 298）：literature:tasks 的 converted / term-generated → 立即 processNoteNow
...
describe('文献笔记生成即跑：literature:tasks → 立即建链', () => {
  beforeEach(() => {
    resetObsidianMocks();
    clearDomainEvents();
    document.body.innerHTML = '';
  ...
  it('converted 事件立即触发：不等防抖窗口，且不受 linkAgentScopes 范围限制', async () => {
    ...
    watcher.start();
    emitDomainEvent('literature:tasks', { kind: 'converted', url: 'BV1xx411c7mD', notePath: '文献盒/新文献.md' });
    await new Promise((r) => setTimeout(r, 5));
  ...
  it('term-generated 携带 notePath 触发；旧载荷（缺 notePath）忽略', async () => {
    ...
    emitDomainEvent('literature:tasks', {
      kind: 'term-generated',
      term: '习得性无助',
      title: '习得性无助',
  ...
  it('failed 事件与文件不存在：一律不触发（notePath 可能是失败任务的旧值）', async () => {
    ...
    watcher.start();
    emitDomainEvent('literature:tasks', { kind: 'failed', url: 'BV1xx411c7mD', notePath: '文献盒/在.md' });
    emitDomainEvent('literature:tasks', { kind: 'converted', url: 'BV1xx411c7mD', notePath: '文献盒/没了.md' });
    emitDomainEvent('literature:tasks', { kind: 'added', url: 'BV1xx411c7mD', notePath: '文献盒/在.md' });
    await new Promise((r) => setTimeout(r, 5));
    expect(agent.processNoteNow).not.toHaveBeenCalled();
    watcher.destroy();
  });
